...
std::ostream& operator<<(std::ostream& os, const Triangle& obj) {
	double p = double(obj.side_a + obj.side_b + obj.side_c) / 2.0;
	os << "Треугольник со сторонами ";
	os << "[" << obj.side_a << "," << obj.side_b << "," << obj.side_c << "], Площадь = " << sqrt(p * (p - double(obj.side_a))*(p - double(obj.side_b))*(p - double(obj.side_c))) << std::endl;
	return os;
}
